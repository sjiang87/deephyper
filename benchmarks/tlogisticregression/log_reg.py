...
def run(param_dict):
    ...
    optimizer= SGD(lr=keras.backend.exp(param_dict['lr']))
    pprint(param_dict)
    ...
    if model is None:
        model = Sequential()
        model.add(Dense(num_classes, input_shape=(784,), activation='softmax', kernel_regularizer=L1L2(l1=0.0, l2=REGULARIZATION))) 
        model.summary()
    ...
    return (-score[1], end_time-start_time)


def augment_parser(parser):

    parser.add_argument('--regularization', action='store', dest='regularization',
                        nargs='?', const=2, type=float, default='0.5',
