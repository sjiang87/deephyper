...
def get_result_random_simple(ROOT_DIR, DATA_DIR, datasets, split_types, range_seeds):
    out_result = {}
    for dataset in datasets:
        for split_type in split_types:
            sizes = (0.5, 0.2, 0.3) if split_type == "523" else (0.8, 0.1, 0.1)
            for seed in range_seeds:
                ...
                file_list = os.path.join(
                    ROOT_DIR,
                    f"NEW_SIMPLE_RANDOM_RESULT/simplepost_model_{dataset}_random_{seed}_split_{split_type}/test_*.pickle",
                )
                file_list = sorted(glob.glob(file_list))
