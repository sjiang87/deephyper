...
# Atom feature sizes
MAX_ATOMIC_NUM = 100
ATOM_FEATURES = {"atomic_num": list(range(MAX_ATOMIC_NUM))}
...
ATOM_FDIM = 1  # sum(len(choices) + 1 for choices in ATOM_FEATURES.values()) + 2
BOND_FDIM = 1  # 14
...
def atom_featurizer(atom):
    ...
    features = (
        atom.GetAtomicNum()
    )  # onek_encoding_unk(atom.GetAtomicNum() - 1, max_atom_type)
    return [features]
...
def get_data_simple(path, tasks, max_data_size=None):
    """
    ...
    # if already exists
    pickle_path = path.replace(".csv", "_simple.pickle")
    # pickle_old_path = path.replace(".csv", '.pickle')

    if os.path.exists(pickle_path):
        with open(pickle_path, "rb") as f:
            data = pickle.load(f)
    else:
        # with open(pickle_old_path, "rb") as handle:
        #     data_old = pickle.load(handle)
        # max_atom_type = len(np.where(data_old["atom_features"][..., :101].sum(axis=(0, 1)) != 0)[0])
        # load data
...
def split_data_simple(
    data, split_type="random", sizes=(0.5, 0.2, 0.3), seed=0, show_mol=False
):
    """
    ...
    if split_type == "random":
        ...
        atom_features_ = np.random.RandomState(seed).permutation(data["atom_features"])
        bond_features_ = np.random.RandomState(seed).permutation(data["bond_features"])
        pair_indices_ = np.random.RandomState(seed).permutation(data["pair_indices"])
        masks_ = np.random.RandomState(seed).permutation(data["masks"])
        label_ = np.random.RandomState(seed).permutation(data["label"])

        x_train = [
            atom_features_[:train_size],
        ...
        x_valid = [
            atom_features_[train_size:train_val_size],
        ...
        mol_ = np.random.RandomState(seed).permutation(np.array(data["smiles"]))
