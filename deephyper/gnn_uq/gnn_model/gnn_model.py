...
from deephyper.gnn_uq.layers import (
    SPARSE_MPNN,
    SPARSE_MPNN_DP,
    GlobalAttentionPool,
...
class RegressionUQSpace_MCDropout(KSearchSpace):
    """
    Search space for regression models with uncertainty quantification.
    ...
    def build(self):
        ...
        out_sub_graph = self.build_sub_graph(self.input_nodes, self.num_layers)
        output_dim = self.output_shape[0]
        output_dropout = ConstantNode(op=Dropout(0.1))
        output_dense = ConstantNode(op=Dense(output_dim * 2))
        self.connect(out_sub_graph, output_dropout)
        self.connect(output_dropout, output_dense)
...
if __name__ == "__main__":
    input_shape = [(8, 133), (33, 2), (33, 14), (8,)]
    output_shape = (1,)
