...
#         super(MP_layer, self).__init__(self)
    ...
    def call(self, inputs, **kwargs):
        ...
        agg_m = self.message_passer([X, A, E])
...
#         super(Message_Passer_NNM, self).__init__()
    ...
    def build(self, input_shape):
        self.nn1 = tf.keras.layers.Dense(units=128,
                                         activation=tf.nn.relu)

        self.nn2 = tf.keras.layers.Dense(units=128,
                                         activation=tf.nn.relu)
    ...
    def call(self, inputs, **kwargs):
        ...
        attn_coef = self.attn_func([X, N, targets, sources])
...
####
class SPARSE_MPNN_DP(tf.keras.layers.Layer):
    r"""Message passing cell.
    Args:
        state_dim (int): number of output channels.
        T (int): number of message passing repetition.
    ...
    def build(self, input_shape):
        ...
        self.dp = tf.keras.layers.Dropout(rate=0.1)
        self.MP = MP_layer_DP(
            self.state_dim,
            self.aggr_method,
            self.activation,
...
class MP_layer_DP(tf.keras.layers.Layer):
    r"""Message passing layer.
    Args:
        state_dim (int): number of output channels.
        attn_heads (int): number of attention heads.
    ...
    def build(self, input_shape):
        self.message_passer = Message_Passer_NNM_DP(
            self.state_dim,
            self.attn_head,
            self.attn_method,
            self.aggr_method,
...
class Message_Passer_NNM_DP(tf.keras.layers.Layer):
    r"""Message passing kernel.
    Args:
        state_dim (int): number of output channels.
        attn_heads (int): number of attention heads.
    ...
    def build(self, input_shape):
        self.nn1 = tf.keras.layers.Dense(units=128, activation=tf.nn.relu)

        self.nn2 = tf.keras.layers.Dense(units=128, activation=tf.nn.relu)
        ...
        self.drop2 = tf.keras.layers.Dropout(rate=0.1)
        self.drop3 = tf.keras.layers.Dropout(rate=0.1)
