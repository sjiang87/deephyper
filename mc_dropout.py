...
def post_train(args, arch, name, idx):
    ...
    POST_RESULT_PICKLE_0 = os.path.join(
        POST_RESULT_DIR,
        f"test_{name}_0.pickle",
    )
    POST_RESULT_PICKLE = os.path.join(
        POST_RESULT_DIR,
        f"test_{name}_{idx}.pickle",
    )
    POST_RESULT_VAL_PICKLE = os.path.join(
        POST_RESULT_DIR,
        f"val_{name}_{idx}.pickle",
    )
    ...
    if not os.path.exists(POST_RESULT_PICKLE_0):
...
def main():
    ...
    for i in range(1):
        ...
        for idx in range(10):
            post_train(args, arch_min[i], id_min[i], idx)
