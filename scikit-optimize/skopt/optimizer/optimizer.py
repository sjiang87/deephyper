...
from ..acquisition import int_gaussian_acquisition
...
class Optimizer(object):
    ...
    * `space`
    ...
    """
    def __init__(self, dimensions, base_estimator="gp", nu=2.5,
                 use_int_acq= False, normalize_dimension = False,
        ...
        # Store parameters for integrated acquisition function
        self.nu = nu
        self.use_int_acq = use_int_acq     
        ...
        if isinstance(self.base_estimator_, GaussianProcessRegressor) and normalize_dimension:
        ...
        if (fit and self._n_initial_points <= 0 and
           self.base_estimator_ is not None):
            ...
            for cand_acq_func in self.cand_acq_funcs_:
                if self.use_int_acq:
                  values = int_gaussian_acquisition(X=X,model=est, y_opt=np.min(self.yi),acq_func=cand_acq_func,
                              acq_func_kwargs=self.acq_func_kwargs )
                else:  
